...
struct b_node {
    ...
    b_node(const std::vector<std::string> & words = {}, const std::vector<std::vector<std::string>>& definitions = {}, const int &n = 0,
        const std::vector<b_node*> &children = std::vector<b_node*>(6, nullptr), b_node * parent = nullptr): words(words), definitions(definitions),
...
class b_tree {
    ...
    bool insert_recurse(const std::string &term, std::vector<std::string> &defs, b_node *root_node);
    ...
    bool insert(const std::string &term, std::vector<std::string> &defs);
